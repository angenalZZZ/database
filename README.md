 ...
 * `DDL`（data definition language）数据库定义语言 [`✨建表规约`](#数据库设计)
...
set @bakfile='d:\database_bak\dbName_log_'+convert(nvarchar(8),getdate(),112)+'.bak'
DBCC SHRINKFILE (N'DBNAME_log', 1, TRUNCATEONLY);
BACKUP LOG dbName TO DISK = @bakfile WITH RETAINDAYS = 1, COMPRESSION;
BACKUP LOG dbName TO DISK = @bakfile WITH NOFORMAT, NOINIT, 
	NAME = N'dbname_log', NOSKIP, NOREWIND, NOUNLOAD, NORECOVERY, STATS=5;

-- 恢复数据库
RESTORE DATABASE [dbname] FROM DISK = N'd:\database_bak\dbName.bak' WITH FILE=1,
	MOVE N'dbname_log' TO N'C:\Program Files\*\log\dbname_log.ldf', NOUNLOAD, STATS=5;
...
> ALTER DATABASE OPEN;
#> ALTER DATABASE CHARACTER SET US7ASCII;          # 更改默认字符集，中文数据会产生乱码
...
> select userenv('language') from dual;            # 验证默认字符集 ZHS16GBK
# shutdown immediate; exit;                        # 关闭数据库再退出
> exit;                                            # 直接退出
...
#### 一、建表规约
	...
	【强制】（3）必须使用utf8mb4字符集
	解读：在Mysql中的UTF-8并非“真正的UTF-8”，而“utf8mb4”才是真正的“UTF-8”。

	【强制】（4）数据库表、表字段必须加入中文注释
	解读：大家都别懒，这是SQL可阅读的基本要求。

	【强制】（5）库名、表名、字段名“均小写”，“下划线风格”，“不超过32个字符”，必须见名知意，“禁止拼音英文混用”。
	解读：约定

	【强制】（6）单表列数目必须“小于30”，若超过则应该考虑将表拆分
...
#### 二、SQL规约
	...
	【强制】（3）不允许使用属性隐式转换
	 解读：假设我们在手机号列上添加了索引，然后执行下面的SQL会发生什么？
	  explain SELECT user_name FROM parent WHERE phone=13812345678; 很明显就是索引不生效，“数字转字符串”导致全表扫描。
	...
	【建议】（6）应尽量避免在WHERE子句中使用“OR”作为连接条件
	 解读：根据情况可以选择使用 UNION ALL 来代替 OR

	【强制】（7）不允许使用“%”开头的模糊查询
	 解读：根据索引的最左前缀原理，%开头的模糊查询无法使用索引，可以使用ElasticSearch来做检索。
...
#### 三、索引规约
	...
	【强制】（2）JOIN的表不允许超过三个。需要JOIN的字段，“数据类型”必须“绝对一致”; 多表关联查询时，保证“被关联的字段”有“索引”。
	...
	【建议】（6）在较长VARCHAR字段,例如VARCHAR(100)上建立索引时，应指定索引长度，没必要对全字段建立索引，根据实际文本区分度决定索引长度即可。
	 解读：索引的长度与区分度是一对矛盾体，一般对字符串类型数据，若长度为20的索引，区分度会高达90%以上，则可以考虑创建长度例为20的索引，
	  ...
	  来确定 lesson_code 字段字符长度为 20 时文本区分度。
...
#### 工具
* [`Sybase PowerDesigner 16`桌面软件](https://shareappscrack.com/sap-powerdesigner/)
* [`tbls`命令行](https://github.com/k1LoW/tbls)输出`Markdown`、`Markdown`、`Word.DOT`、`Excel`、`Plant.UML`、`Image`、`JSON`、`YAML`
